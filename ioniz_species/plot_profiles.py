...
for j in range(len(labels)):
    ...
    Novi = np.loadtxt('Novi_thrutime/'+labels[j]+'_Novi_'+time+'.np')
    R = np.loadtxt('Rbins_thrutime/'+labels[j]+'_Rbins_'+time+'.np')
...
for j in range(len(labels)):
    ...
    M_ox = np.loadtxt('Omass_thrutime/'+labels[j]+'_Omass_'+time+'.np')
    R = np.loadtxt('Rbins_thrutime/'+labels[j]+'_Rbins_'+time+'.np')
