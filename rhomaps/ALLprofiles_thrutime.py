...
#    print(h1.g['t_cool'])
#    quit()
    sph.image(h1.g,qty="rho",units='g cm^-2',width=400,cmap="Blues",log=True,qtytitle=r'$\rho$ [g cm$^{-2}$]')
    print(np.average(h1.g['rho'].in_units('g cm^-2')),np.max(h1.g['rho'].in_units('g cm^-2')))
    plt.title('z = '+str('%.2f' % f.properties['z'])+', '+str('%.2f' % f.properties['time'].in_units('Gyr'))+' Gyr')
    plt.savefig(labels[k]+'_rhomap'+ts[i]+'.pdf')
#    plt.show()
