...
GM4_BH_all_hat = GM4_BH.calculate('reassemble(BH_mdot_histogram, "sum")')
#GM4_BH_all_hat = smooth(GM4_BH_all, 50)

plt.plot(P0_BH_time_bins, np.log10(np.cumsum(P0_BH_all_hat*np.diff(P0_BH_time_bins)[0]*10**9)),color='DodgerBlue',label='P0')
plt.plot(GM1_BH_time_bins, np.log10(np.cumsum(GM1_BH_all_hat*np.diff(GM1_BH_time_bins)[0]*10**9)),color='SteelBlue',label='GM1')
plt.plot(GM7_BH_time_bins, np.log10(np.cumsum(GM7_BH_all_hat*np.diff(GM7_BH_time_bins)[0]*10**9)),color='FireBrick',label='GM2')
plt.plot(GM4_BH_time_bins, np.log10(np.cumsum(GM4_BH_all_hat*np.diff(GM4_BH_time_bins)[0]*10**9)),color='Salmon',label='GM3')
plt.xlabel("Age/Gyr")
plt.ylabel("Log Cumulative BH Accreted Mass/$M_{\odot}$")
plt.ylim(3,8)
plt.legend()
