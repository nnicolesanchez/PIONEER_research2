# This script prints out metallicity images of the merger
# at z ~ 0.2 in the noBH PIONEER galaxies 
...
ts = np.loadtxt('../'+str(sys.argv[1])+'/timesteps.txt',dtype='str')

for t in range(10,len(ts)):
    try:
        sim = pynbody.load(simname+ts[t])
        sim.properties
        print(name+' simulation at z = ','%.2f' % sim.properties['z'],' and time = ',sim.properties['time'].in_units("Gyr") )
        sim.physical_units()
        h = sim.halos()
